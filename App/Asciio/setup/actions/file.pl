...
if(defined $file_name && $file_name ne q[])
	...
	eval
        ...
        $new_title = $self->save_with_type($elements_to_save, "txt", $path.$base_name.".txt") ;
        system("java -jar /opt/local/share/java/ditaa0_9.jar \"".$path.$base_name.".txt\" \"".$path.$base_name.".png\"");
		} ;
